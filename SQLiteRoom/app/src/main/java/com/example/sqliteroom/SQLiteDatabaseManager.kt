...
class SQLiteDatabaseManager(private val context: Context) {
    ...
    fun insertValue(employeeDataClass: EmployeeDataClass) {
        ...
        // Return numerical value and returns id else return -1
    ...
    fun getAllEmpDataFromSQLiteDB(): ArrayList<EmployeeDataClass> {
        ...
        // Rawquery to exceute select query and it returns a result set in cursor
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                employee.id =
                    cursor.getInt(cursor.getColumnIndex(SQLiteDatabaseHelper.COLUMN_ID)) //getColumnIndex returns index of COLUMN_ID
                employee.name =
